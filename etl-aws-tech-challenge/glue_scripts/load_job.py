...
df = df.withColumn('Corpo de Candle', col('Fechamento') - col('Abertura'))
df = df.withColumn(
    'Sombra Superior',
    when(col('Corpo de Candle') >= 0, col('Máximo') - col('Fechamento'))
    .otherwise(col('Máximo') - col('Abertura'))
)
df = df.withColumn(
    'Sombra Inferior',
    when(col('Corpo de Candle') >= 0, col('Mínimo') - col('Abertura'))
    .otherwise(col('Mínimo') - col('Fechamento'))
)
...
df = df.withColumn(
    'Média Móvel',
    round(avg(col('Fechamento')).over(windowSpec.rowsBetween(-(df.count() - 1), 0)), 2)
)
...
transformed_table_name = 'b3_final'
...
table_input = {
    ...
    'StorageDescriptor': {
        'Columns': [
            ...
            {'Name': "Corpo de Candle", 'Type': 'double'},
            {'Name': "Sombra Superior", 'Type': 'double'},
            {'Name': "Sombra Inferior", 'Type': 'double'},
            {'Name': "Média Móvel", 'Type': 'double'}
        ],
